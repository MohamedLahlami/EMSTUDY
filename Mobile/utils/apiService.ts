...
class ApiService {
  ...
  private handleError(error: any): never {
    ...
    if (error.name === "AbortError") {
      throw new Error(
        "La requête a expiré. Vérifiez votre connexion ou le serveur."
      );
    } else if (
      ...
      throw new Error(
        "Connexion au serveur impossible. Vérifiez que votre backend est démarré sur le port 8080."
      );
    }
  ...
  private async request<T>(
    ...
    try {
      ...
      if (
        ...
        if (Platform.OS === "web") {
          errorMessage +=
            " Vérifiez que votre backend est démarré sur 192.168.11.170:8080.";
        } else if (Platform.OS === "android") {
          errorMessage +=
            " Si vous utilisez un émulateur Android, assurez-vous que votre backend est accessible sur 10.0.2.2:8080.";
        } else if (Platform.OS === "ios") {
          errorMessage +=
            " Si vous utilisez un simulateur iOS, assurez-vous que votre backend est accessible sur 192.168.11.170:8080.";
        } else {
