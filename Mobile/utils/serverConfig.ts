...
class ServerConfig {
  ...
  getBaseUrl(): string {
    ...
    if (Platform.OS === 'web') {
      ...
      // Pour les émulateurs Android, 10.0.2.2 correspond à localhost de la machine hôte
  ...
  async init(): Promise<void> {
    try {
      ...
      if (savedUrl) {
        ...
        if (this._debugMode) {
          console.log(`[ServerConfig] URL chargée depuis le stockage: ${savedUrl}`);
        }
      }
    } catch (error) {
      console.error('[ServerConfig] Erreur lors du chargement de la config:', error);
    }
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      ...
      try {
        ...
        if (this._debugMode) {
          console.log(`[ServerConfig] Réponse du serveur: ${response.status} ${response.statusText}`);
        }
