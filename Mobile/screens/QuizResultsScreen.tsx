...
export default function QuizResultsScreen() {
  ...
  const fetchSubmissionDetails = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/submissions/${submissionId}`, {
        headers: {
        ...
        {submission && (
          <View style={styles.resultsContainer}>
            <View style={styles.scoreCard}>
              ...
              <Text style={styles.scoreValue}>{Math.round(submission.score)}%</Text>
            </View>
                ...
                {submission.quiz.questions.map((question, index) => (
                  <View key={question.questionId} style={styles.questionCard}>
                    ...
                    {question.answers.map(answer => {
                      const isSelected = submission.answers.some(a => a.answerId === answer.answerId);
                      ...
                      return (
                        ...
                        >
                          <MaterialIcons
                            name={isSelected ? (isCorrect ? 'check-circle' : 'cancel') : 'radio-button-unchecked'}
                            size={24}
                            color={isSelected ? (isCorrect ? '#4caf50' : '#f44336') : '#666'}
                          />
