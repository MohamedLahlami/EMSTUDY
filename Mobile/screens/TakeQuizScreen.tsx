...
export default function TakeQuizScreen() {
  ...
  const fetchQuizDetails = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/items/${quizId}`, {
        headers: {
      ...
      // Start quiz
      const submissionResponse = await fetch(`http://localhost:8080/submissions/start/?quizId=${quizId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${authData?.token}`,
        },
      ...
      // Fetch questions
      const questionsResponse = await fetch(`http://localhost:8080/questions/quiz/${quizId}`, {
        headers: {
          'Authorization': `Bearer ${authData?.token}`,
        },
      });
  ...
  const submitQuiz = async () => {
    try {
      ...
      await fetch(`http://localhost:8080/submissions/${submission.submissionId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${authData?.token}`,
      ...
      navigation.replace('QuizResults', { submissionId: submission.submissionId });
    } catch (error) {
        ...
        {question.answers.map(answer => {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => handleAnswerSelection(question.questionId, answer.answerId, isMultiSelect)}
            >
              <MaterialIcons
                name={isMultiSelect ? (isSelected ? 'check-box' : 'check-box-outline-blank') : (isSelected ? 'radio-button-checked' : 'radio-button-unchecked')}
                size={24}
